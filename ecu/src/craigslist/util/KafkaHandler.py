...
class KafkaHandler(logging.Handler):
    ...
    Email on exceptions raised from delivery failure is supported with SMTP"""

    def __init__(self, backup_file, hosts, zookeeper, topic, batch_size, mailhost, fromaddr,
                 toaddrs, subject, credentials=None, secure=None, timeout=5.0):
    ...
    def emit(self, record):
        ...
        try:
            ...
            self.write_backup("ERROR: Kafka Producer failed to back up to topic:",
                              + str(self.topic))
    ...
    def getSubject(self, exception):
        ...
        """
        # RFC 2822 Section 2.1.1: Email subject length is limited to the 78 character length
        # standard
        return self.subject + " " + self.topic + " " + exception \
            if len(self.subject) + len(exception) + len(self.topic) < 79 \
                    ...
                    try:
                        ...
                        if self.username:
                            ...
                            if self.secure is not None:
                                smtp.ehlo()
                                # could potentially throw error if smtp server does not support
                                # STARTTLS or a snake does a man in the middle attack
